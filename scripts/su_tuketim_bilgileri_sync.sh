...
load_config() {
    ...
    ORACLE_TABLE="${SU_TUKETIM_BILGILERI_TABLE:-SU_TUKETIM_BILGILERI}"
...
SELECT 
    ...
    adi AS "ADI",
    soyadi AS "SOYADI",
    adres_no AS "ADRES_NO",
    kapi_entegrasyon_no AS "KAPI_ENTEGRASYON_NO",
    kapi_maks_no AS "KAPI_MAKS_NO",
    kapi_lokasyonu AS "GEOMETRY",
    bina_entegrasyon_no AS "BINA_ENTEGRASYON_NO",
    bina_maks_no AS "BINA_MAKS_NO",
    tuketim AS "TUKETIM",
    son_tuketim AS "SON_TUKETIM",
    mekanik_endeks AS "MEKANIK_ENDEKS",
    ortalama_sarfiyat AS "ORTALAMA_SARFIYAT"
FROM abys.su_tuketim_bilgileri
EOF
}
